...
describe("scan cache round trip", () => {
  it("restores records unchanged", () => {
    const original = cacheWith([
      ["/a.jsonl", 100, [record(), record({ dedupeKey: "msg_2:req_2", model: "claude-opus-5" })]],
  ...
  it("interns repeated model and session strings", () => {
    const encoded = encodeScanCache(
      cacheWith([["/a.jsonl", 100, [record(), record({ dedupeKey: "msg_2:req_2" }), record()]]]),
    );
  ...
  it("rejects the whole cache when an intern table holds a non-string", () => {
    ...
    const poisoned = {
      ...encodeScanCache(cacheWith([["/a.jsonl", 100, [record()]]])),
      models: [1],
    };
  ...
  it("drops the whole entry when any row is corrupt, forcing a cold re-parse", () => {
    // Keeping the surviving rows under the original (size, mtime) would read as
    // a valid warm hit and the file would never be re-parsed.
    const encoded = encodeScanCache(
      cacheWith([
        ["/a.jsonl", 100, [record(), record({ dedupeKey: "msg_2:req_2" })]],
        ["/good.jsonl", 100, [record()]],
      ]),
    );
...
describe("pruneScanCache", () => {
  ...
  it("drops in-window entries whose file has disappeared but keeps the ones seen", () => {
    const cache = cacheWith([
      ["/gone.jsonl", 5000, [record()]],
      ["/live.jsonl", 5000, [record()]],
    ]);
    ...
    expect([...cache.keys()]).toEqual(["/live.jsonl"]);
  });

  it("keeps entries outside the walked window that are still within retention", () => {
    // Viewing 7 days must not evict the 90-day entries, which that walk never
...
describe("dedupeWithinFile", () => {
  ...
  it("keeps every record that has no dedupe key", () => {
    // Codex records are unique per rollout and carry no key.
    expect(
