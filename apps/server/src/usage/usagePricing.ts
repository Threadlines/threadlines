 ...
 * `*_priority`, `*_batches`) and separate 1h cache-write rates. We deliberately
 * price at the base tier: the transcripts don't record which tier served a
 * request, so anything else would be a guess dressed up as precision.
 */
...
export function parseRateTable(document: unknown): RateTable {
  ...
  for (const [name, raw] of Object.entries(document as Record<string, unknown>)) {
    ...
    table.set(normalized, {
      ...
      // premium. When a model omits them, cached input is priced as plain input
      // rather than as free.
