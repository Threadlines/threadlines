 ...
 * Without this every server restart re-parses the whole window.
 *
...
/**
 * Bump whenever a parser change alters what a given file parses to. Stale
 ...
 * object-per-record: on a long window that is the difference between a cache
 * file measured in tens of megabytes and one a fraction of that size.
 */
...
export function decodeScanCache(document: unknown): ScanCache {
  ...
  if (!isArray(root.models) || !isArray(root.sessions)) return cache;
  ...
  // undefined guard below, land in a record's model, and crash the aggregate at
  // normalizeModelName. A corrupt table rejects the whole cache.
  ...
  for (const [path, raw] of Object.entries(root.files)) {
    ...
    if (!isArray(entry.r)) continue;

    const provider: UsageProviderKind = entry.p;
    const records: UsageRecord[] = [];
    // Any corrupt row disqualifies the whole entry. Keeping the survivors under
    // the original (size, mtime) would read as a valid warm hit and the file
 ...
 * missed would evict the 90-day entries every time someone looked at 7 days.
 */
