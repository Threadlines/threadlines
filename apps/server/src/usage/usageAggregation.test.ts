...
describe("UsageAggregator", () => {
  ...
  it("prices against the rate table", () => {
    ...
    // Cached input at full input rates minus what it actually cost.
  ...
  it("drops records outside the window and reports whether one contributed", () => {
    const aggregator = new UsageAggregator({
      timeZone: "UTC",
      sinceDay: "2026-08-01",
      untilDay: "2026-08-31",
    ...
    expect(aggregator.add(record({ dedupeKey: "msg_1:req_1" }))).toBe(true);
    expect(aggregator.add(record({ dedupeKey: "msg_1:req_1" }))).toBe(false);
    // The mtime prefilter admits boundary files whose records fall outside.
    expect(aggregator.add(record({ timestampMs: Date.parse("2026-07-31T23:59:59Z") }))).toBe(false);
    expect(aggregator.add(record({ timestampMs: Date.parse("2026-09-01T00:00:00Z") }))).toBe(false);
    expect(aggregator.add(record({ timestampMs: Date.parse("2026-08-01T00:00:00Z") }))).toBe(true);

    expect(aggregator.finish().outOfWindow).toBe(2);
  });
