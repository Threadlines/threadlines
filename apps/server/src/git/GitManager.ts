...
export class GitManager extends Context.Service<GitManager, GitManagerShape>()(
  "threadlines/git/GitManager",
) {}

const COMMIT_TIMEOUT_MS = 10 * 60_000;
const COMMIT_MESSAGE_SUMMARY_CONTEXT_MAX_CHARS = 4_000;
const COMMIT_MESSAGE_PATCH_CONTEXT_MAX_CHARS = 8_000;
...
function resolvePullRequestWorktreeLocalBranchName(
  ...
  return `threadlines/pr-${pullRequest.number}/${suffix}`;
}
...
export const makeGitManager = Effect.fn("makeGitManager")(function* () {
  ...
  const captureGitIndexSnapshot = Effect.fn("captureGitIndexSnapshot")(function* (cwd: string) {
    ...
    const backupPath = path.join(tempDir, `threadlines-git-index-${process.pid}-${randomUUID()}`);
  ...
  const resolveCommitAndBranchSuggestion = Effect.fn("resolveCommitAndBranchSuggestion")(
    function* (input: {
      ...
      const context = yield* input.preview
        ...
        : gitCore.prepareCommitContext(input.cwd, input.filePaths);
      if (!context) {
        return null;
      }
