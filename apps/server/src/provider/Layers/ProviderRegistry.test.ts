...
import { deepMerge } from "@threadlines/shared/Struct";
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest(), TestHttpClientLive))(
  ...
  (it) => {
    ...
    describe("ProviderRegistryLive", () => {
      ...
      it.effect("propagates real Codex probe failures to the aggregator at boot", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            const registry = yield* ProviderRegistry;
            // The probe runs in the instance's background fiber, so poll
            // until its failure snapshot lands in the aggregator.
            let providers = yield* registry.getProviders;
            for (
              let attempts = 0;
              attempts < 50 &&
      ...
      it.effect("re-probes when settings change the codex binaryPath", () =>
        Effect.gen(function* () {
          ...
          yield* Effect.gen(function* () {
            ...
            // snapshot should be `status: "error"`. The probe runs in the
            // instance's background fiber, so poll until it lands. What
            // *distinguishes* the two probe runs is `checkedAt` — each
            // probe stamps a fresh DateTime, so we capture it and assert
            // it advances after the settings mutation.
            ...
            // fires `syncLiveSources`, which subscribes and lets the
            // rebuilt instance's background probe land via the stream.
    ...
    describe("checkClaudeProviderStatus", () => {
      it.effect("does not treat a fresh Claude model catalog as configured credentials", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            ...
            {},
            () => Effect.die("Signed-out accounts must not probe subscription usage"),
          );
          assert.strictEqual(status.installed, true);
          assert.strictEqual(status.version, "2.1.261");
          assert.strictEqual(status.status, "warning");
          assert.strictEqual(status.auth.status, "unauthenticated");
          assert.strictEqual(status.auth.capabilities, undefined);
          ...
          Effect.provide(
            mockSpawnerLayer((args) => {
              const joined = args.join(" ");
              if (joined === "--version") return { stdout: "2.1.261\n", stderr: "", code: 0 };
              if (joined === "auth status")
                return {
                  stdout: JSON.stringify({
                    ...
                    projectsDirectory: "C:\\Users\\threadlines\\.claude\\projects",
                  }),
                  stderr: "",
                  code: 1,
                };
      ...
      it.effect("includes Claude Fable 5.1 with launch capabilities on supported versions", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
          ...
          assert.strictEqual(
            fable51.capabilities.optionDescriptors?.some(
      ...
      it.effect("hides Claude Fable 5.1 before the Claude Code version that exposes it", () =>
        Effect.gen(function* () {
          const status = yield* checkClaudeProviderStatus(
            defaultClaudeSettings,
            claudeCapabilities(),
      ...
      it.effect("runs Claude status probes with the configured Claude HOME", () => {
        const claudeHome = "/tmp/threadlines-claude-home";
        ...
        return Effect.gen(function* () {
          ...
          assert.deepStrictEqual(
            ...
            [path.resolve(claudeHome), path.resolve(claudeHome)],
          );
      ...
      it.effect(
        "reports signed out when Claude initialization is unavailable and auth is false",
        () =>
          Effect.gen(function* () {
            const status = yield* checkClaudeProviderStatus(
              defaultClaudeSettings,
