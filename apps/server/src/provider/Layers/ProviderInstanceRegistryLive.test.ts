...
const homeGroupKey = (
  path: { readonly resolve: (path: string) => string },
  driver: string,
  homePath: string,
) => `${driver}:home:${path.resolve(homePath)}`;
  ...
  it.live("boots two independent codex instances from a ProviderInstanceConfigMap", () =>
    Effect.gen(function* () {
      ...
      expect(personalSnapshot.continuation?.groupKey).toBe(
        homeGroupKey(path, "codex", "/home/julius/.codex_personal"),
      );
      ...
      expect(workSnapshot.continuation?.groupKey).toBe(
        homeGroupKey(path, "codex", "/home/julius/.codex"),
      );
  ...
  it.live("boots one instance of every shipped driver from a single config map", () =>
    Effect.gen(function* () {
      ...
      expect(claudeSnapshot.continuation?.groupKey).toBe(
        homeGroupKey(path, "claude", "/home/julius/.claude-work"),
      );
