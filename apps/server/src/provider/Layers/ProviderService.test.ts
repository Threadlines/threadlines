...
function makeFakeCodexAdapter(provider: ProviderDriverKind = CODEX_DRIVER) {
  ...
  const clearThreadGoal = vi.fn((threadId: ThreadId): Effect.Effect<void, ProviderAdapterError> =>
    Effect.sync(() => {
  ...
  const deleteThread = vi.fn((threadId: ThreadId): Effect.Effect<void, ProviderAdapterError> =>
    Effect.sync(() => {
      sessions.delete(threadId);
    }),
  );
...
fanout.layer("ProviderServiceLive fanout", (it) => {
  ...
  it.effect("records provider metrics with the routed provider label", () =>
    Effect.gen(function* () {
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "threadlines_provider_turns_total", {
          provider: ProviderDriverKind.make("claudeAgent"),
          operation: "interrupt",
          outcome: "success",
        }),
      ...
      assert.equal(
        hasMetricSnapshot(snapshots, "threadlines_provider_sessions_total", {
          provider: ProviderDriverKind.make("claudeAgent"),
          operation: "stop",
          outcome: "success",
        }),
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(
          hasMetricSnapshot(snapshots, "threadlines_provider_turn_duration", {
            provider: ProviderDriverKind.make("claudeAgent"),
            operation: "send",
          }),
          true,
