...
import { PullRequestAutoFixWatcher } from "./orchestration/Services/PullRequestAutoFixWatcher.ts";
...
>()("threadlines/serverRuntimeStartup") {}
...
export const makeServerRuntimeStartup = Effect.gen(function* () {
  ...
  const pullRequestAutoFixWatcher = yield* PullRequestAutoFixWatcher;
  ...
  const startup = Effect.gen(function* () {
    ...
    yield* runStartupPhase(
      ...
      Effect.gen(function* () {
        ...
        yield* pullRequestAutoFixWatcher.start().pipe(Scope.provide(reactorScope));
      ...
      if (serverConfig.startupPresentation === "headless") {
        ...
        if (serverConfig.mode !== "desktop") {
          yield* Effect.logInfo(
            "Authentication required. Open Threadlines using the pairing URL.",
