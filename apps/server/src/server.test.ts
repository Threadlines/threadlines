...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    gitAuthRemediation?: Partial<GitAuthRemediationService.GitAuthRemediationServiceShape>;
  ...
  Effect.gen(function* () {
    ...
    const tempBaseDir = yield* fileSystem.makeTempDirectoryScoped({
      prefix: "threadlines-router-test-",
    });
    ...
    const gitAuthRemediationLayer = Layer.mock(GitAuthRemediationService.GitAuthRemediationService)(
      {
        ...options?.layers?.gitAuthRemediation,
      },
    );
...
it.layer(NodeServices.layer)("server router seam", (it) => {
  it.effect("serves static index content for GET / when staticDir is configured", () =>
    Effect.gen(function* () {
      ...
      const staticDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "threadlines-router-static-",
      });
  ...
  it.effect("serves project favicon requests before the dev URL redirect", () =>
    Effect.gen(function* () {
      ...
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "threadlines-router-project-favicon-",
      });
  ...
  it.effect("serves the fallback project favicon when no icon exists", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const projectDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "threadlines-router-project-favicon-fallback-",
      });
  ...
  it.effect("serves the legacy T3 environment descriptor route for compatibility", () =>
    Effect.gen(function* () {
      yield* buildAppUnderTest();
  ...
  it.effect("reports unauthenticated session state without requiring auth", () =>
    Effect.gen(function* () {
      ...
      assert.isTrue(body.auth.sessionCookieName.startsWith("threadlines_session_"));
    }).pipe(Effect.provide(NodeHttpServer.layerTest)),
  );
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        const fileSystem = yield* FileSystem.FileSystem;
        const projectDir = yield* fileSystem.makeTempDirectoryScoped({
          prefix: "threadlines-router-project-favicon-query-token-",
        });
        ...
        resourceSpans: [
          {
            resource: {
              attributes: [
                {
                  key: "service.name",
                  value: { stringValue: "threadlines-web" },
                },
              ],
            },
            scopeSpans: [
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.equal(record.resourceAttributes["service.name"], "threadlines-web");
    ...
    () =>
      Effect.gen(function* () {
        ...
        assert.deepEqual(createWorktree.mock.calls[0]?.[0], {
          cwd: "/tmp/project",
          refName: "main",
          newRefName: "threadlines/bootstrap-refName",
          path: null,
        });
