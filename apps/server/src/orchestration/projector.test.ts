...
describe("orchestration projector", () => {
  ...
  it("applies thread.pinned and thread.unpinned events", async () => {
    ...
    expect(pinned.threads[0]?.pinnedAt).toBe(later);
  ...
  it("applies thread.pull-request-automation-changed events", async () => {
    const now = "2026-01-01T00:00:00.000Z";
    const later = "2026-01-01T00:00:01.000Z";
    const created = await Effect.runPromise(
      projectEvent(
    ...
    expect(created.threads[0]?.pullRequestAutoFix).toBe(false);

    const armed = await Effect.runPromise(
      projectEvent(
        created,
        makeEvent({
          sequence: 2,
          ...
          aggregateId: "thread-1",
          occurredAt: later,
          commandId: "cmd-thread-auto-fix",
          payload: { threadId: "thread-1", autoFix: true, updatedAt: later },
        }),
      ),
    );
  ...
  it("keeps the latest turn running when a provider diff placeholder arrives mid-turn", async () => {
    const createdAt = "2026-02-23T08:00:00.000Z";
    const startedAt = "2026-02-23T08:00:05.000Z";
    const placeholderAt = "2026-02-23T08:00:07.000Z";
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            status: "missing",
            files: [{ path: "apps/web/src/App.tsx", kind: "modified", additions: 3, deletions: 1 }],
    ...
    expect(afterPlaceholder.threads[0]?.checkpoints[0]?.files).toEqual([
      { path: "apps/web/src/App.tsx", kind: "modified", additions: 3, deletions: 1 },
    ]);
      ...
      projectEvent(
        ...
        makeEvent({
          ...
          payload: {
            ...
            turnId: "turn-1",
            checkpointTurnCount: 1,
            checkpointRef: "refs/threadlines/checkpoints/thread-1/turn/1",
            status: "ready",
    ...
    expect(afterComplete.threads[0]?.checkpoints[0]?.status).toBe("ready");
  });
    ...
    const events: ReadonlyArray<OrchestrationEvent> = [
      ...
      makeEvent({
        ...
        payload: {
          ...
          checkpointRef: "refs/threadlines/checkpoints/thread-1/turn/2",
          status: "ready",
          files: [],
    ...
    const events: ReadonlyArray<OrchestrationEvent> = [
      makeEvent({
        ...
        payload: {
          ...
          turnId: "turn-1",
          checkpointTurnCount: 1,
          checkpointRef: "refs/threadlines/checkpoints/thread-revert/turn/1",
          status: "ready",
          files: [],
      ...
      makeEvent({
        ...
        payload: {
          ...
          turnId: "turn-2",
          checkpointTurnCount: 2,
          checkpointRef: "refs/threadlines/checkpoints/thread-revert/turn/2",
          status: "ready",
          files: [],
      ...
      (_, index) =>
        makeEvent({
          ...
          payload: {
            ...
            checkpointRef: `refs/threadlines/checkpoints/thread-capped/turn/${index + 1}`,
            status: "ready",
            files: [],
