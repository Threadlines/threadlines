...
function makeEvent(input: {
  ...
  return {
    ...
    type: input.aggregateKind === "project" ? "project.meta-updated" : "thread.meta-updated",
...
describe("coalesceLatestAggregateEvents", () => {
  it.effect("keeps only the latest event per aggregate within a window", () =>
    Effect.gen(function* () {
  ...
  it.effect("flushes per window while the source stays open", () =>
    Effect.gen(function* () {
      const firstBatch = [
        makeEvent({ sequence: 1, aggregateKind: "thread", aggregateId: "thread-a" }),
        makeEvent({ sequence: 2, aggregateKind: "thread", aggregateId: "thread-a" }),
  ...
  it.effect("emits nothing extra for an empty source", () =>
    Effect.gen(function* () {
