...
const decodeProjectCatalog = Schema.decodeUnknownEffect(OrchestrationProjectCatalogSnapshot);
...
const ProjectionProjectCatalogDbRowSchema = Schema.Struct({
  projectId: ProjectionProject.fields.projectId,
  kind: ProjectionProject.fields.kind,
 ...
 * shell payload never has to carry the file lists themselves.
 */
const ProjectionThreadDiffStatDbRowSchema = Schema.Struct({
  threadId: ThreadId,
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
  ...
  const listProjectCatalogRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionProjectCatalogDbRowSchema,
    execute: () =>
      sql`
        SELECT
          project_id AS "projectId",
    ...
    execute: () =>
      sql`
        SELECT
          ...
          effective_cwd_source AS "effectiveCwdSource",
          goal AS "goal",
  ...
  const listThreadMessageRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        ...
        ORDER BY thread_id ASC, event_sequence ASC, created_at ASC, message_id ASC
      `,
  });

  // The command read model hydrates the same per-thread message tail the
  // projector keeps in memory (MAX_THREAD_MESSAGES) so decider invariants
  // that consult message history (turn retry, fork source lookup) behave the
  // same after a server restart as they do on a long-running process.
  const listCappedThreadMessageRows = SqlSchema.findAll({
    Request: Schema.Void,
    Result: ProjectionThreadMessageDbRowSchema,
    execute: () =>
      sql`
  ...
  const listThreadProposedPlanRows = SqlSchema.findAll({
    ...
    execute: () =>
      sql`
        ...
        ORDER BY thread_id ASC, event_sequence ASC, created_at ASC, plan_id ASC
      `,
  });
  ...
  const listThreadSubagentRows = SqlSchema.findAll({
    ...
    execute: () => sql`
      ...
      ORDER BY thread_id ASC, created_at ASC, subagent_id ASC
    `,
  });
  ...
  // shared checkout, say), and the turn timeline renders those files the same
  // as a "ready" capture. Filtering here would make the sidebar disagree with
  ...
  const listThreadSubagentRowsByThread = SqlSchema.findAll({
    Request: ThreadIdLookupInput,
    Result: ProjectionThreadSubagentDbRowSchema,
    execute: ({ threadId }) =>
      sql`
        SELECT
          thread_id AS "threadId",
          subagent_id AS "id",
          agent_thread_id AS "agentThreadId",
          parent_agent_thread_id AS "parentAgentThreadId",
          spawn_call_id AS "spawnCallId",
          ...
          result_created_at AS "resultCreatedAt",
          result_event_sequence AS "resultEventSequence",
          live_event_sequence AS "liveEventSequence",
          created_at AS "createdAt",
          updated_at AS "updatedAt"
        FROM projection_thread_subagents
        WHERE thread_id = ${threadId}
  ...
  const getProjectCatalog: ProjectionSnapshotQueryShape["getProjectCatalog"] = () =>
    sql
      .withTransaction(
        Effect.all([
          listProjectCatalogRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getProjectCatalog:listProjects:query",
                "ProjectionSnapshotQuery.getProjectCatalog:listProjects:decodeRows",
              ),
            ),
          ),
          listProjectionStateRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getProjectCatalog:listProjectionState:query",
                "ProjectionSnapshotQuery.getProjectCatalog:listProjectionState:decodeRows",
              ),
            ),
          ),
        ]),
      ...
      .pipe(
        Effect.flatMap(([projectRows, stateRows]) => {
          ...
          return decodeProjectCatalog({
            ...
            Effect.mapError(
              toPersistenceDecodeError(
                "ProjectionSnapshotQuery.getProjectCatalog:decodeProjectCatalog",
              ),
            ),
          );
        }),
        Effect.mapError((error) => {
          ...
          return toPersistenceSqlError("ProjectionSnapshotQuery.getProjectCatalog:query")(error);
        }),
      );
        ...
        Effect.flatMap(
          ([
            ...
            Effect.gen(function* () {
              ...
              const subagentsByThread = new Map<string, Array<OrchestrationSubagent>>();
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                pullRequestAutoMerge: row.pullRequestAutoMerge ?? null,
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listCappedThreadMessageRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
    ...
    sql
      .withTransaction(
        Effect.all([
          ...
          listThreadDiffStatRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getShellSnapshot:listThreadDiffStats:query",
                "ProjectionSnapshotQuery.getShellSnapshot:listThreadDiffStats:decodeRows",
              ),
            ),
          ),
        ]),
      ...
      .pipe(
        Effect.flatMap(
          ([projectRows, threadRows, sessionRows, latestTurnRows, stateRows, diffStatRows]) =>
            Effect.gen(function* () {
              let updatedAt: string | null = null;
              for (const row of projectRows) {
                updatedAt = maxIso(updatedAt, row.updatedAt);
              ...
              const diffStatByThread = new Map(
                diffStatRows.map((row) => [row.threadId, row] as const),
              );
      ...
      .withTransaction(
        Effect.all([
          ...
          listThreadDiffStatRows(undefined).pipe(
            Effect.mapError(
              toPersistenceSqlOrDecodeError(
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listThreadDiffStats:query",
                "ProjectionSnapshotQuery.getArchivedShellSnapshot:listThreadDiffStats:decodeRows",
              ),
            ),
          ),
        ]),
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      const [threadRow, latestTurnRow, sessionRow, diffStatRow] = yield* Effect.all([
        ...
        getThreadDiffStatRow({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadShellById:getThreadDiffStat:query",
              "ProjectionSnapshotQuery.getThreadShellById:getThreadDiffStat:decodeRow",
            ),
          ),
        ),
      ]);
      ...
      return Option.some({
        ...
        pullRequestAutoMerge: threadRow.value.pullRequestAutoMerge ?? null,
  ...
  const getThreadDetailById: ProjectionSnapshotQueryShape["getThreadDetailById"] = (threadId) =>
    Effect.gen(function* () {
      const [
        ...
        listThreadSubagentRowsByThread({ threadId }).pipe(
          Effect.mapError(
            toPersistenceSqlOrDecodeError(
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagents:query",
              "ProjectionSnapshotQuery.getThreadDetailById:listSubagents:decodeRows",
            ),
          ),
        ),
