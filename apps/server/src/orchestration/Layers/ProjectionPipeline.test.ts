    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make(
            "refs/threadlines/checkpoints/thread-revert-files/turn/1",
          ),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointRef: CheckpointRef.make(
            "refs/threadlines/checkpoints/thread-revert-files/turn/2",
          ),
          status: "ready",
          files: [],
      ...
      Effect.gen(function* () {
        ...
        yield* appendAndProject({
          ...
          payload: {
            ...
            checkpointTurnCount: 1,
            checkpointRef: CheckpointRef.make(
              "refs/threadlines/checkpoints/thread-conflict/turn/1",
            ),
            status: "ready",
            files: [],
    ...
    Effect.gen(function* () {
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 1,
          checkpointRef: CheckpointRef.make("refs/threadlines/checkpoints/thread-revert/turn/1"),
          status: "ready",
          files: [],
      ...
      yield* appendAndProject({
        ...
        payload: {
          ...
          checkpointTurnCount: 2,
          checkpointRef: CheckpointRef.make("refs/threadlines/checkpoints/thread-revert/turn/2"),
          status: "ready",
          files: [],
...
engineLayer("OrchestrationProjectionPipeline via engine dispatch", (it) => {
  ...
  it.effect("preserves latest turn id when a running session becomes ready", () =>
    Effect.gen(function* () {
      const engine = yield* OrchestrationEngineService;
      const sql = yield* SqlClient.SqlClient;
      const createdAt = "2026-01-01T00:00:00.000Z";
      const threadId = ThreadId.make("thread-completed-status");
      const turnId = TurnId.make("turn-completed-status");

      yield* engine.dispatch({
        type: "project.create",
        commandId: CommandId.make("cmd-completed-status-project"),
        projectId: ProjectId.make("project-completed-status"),
        title: "Completed Status Project",
        workspaceRoot: "/tmp/project-completed-status",
        defaultModelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
          model: "gpt-5-codex",
        },
      ...
      yield* engine.dispatch({
        type: "thread.create",
        commandId: CommandId.make("cmd-completed-status-thread"),
        threadId,
        projectId: ProjectId.make("project-completed-status"),
        title: "Completed Status Thread",
        modelSelection: {
          instanceId: ProviderInstanceId.make("codex"),
          model: "gpt-5-codex",
        },
      ...
      yield* engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-completed-status-running"),
        threadId,
        session: {
          threadId,
          status: "running",
      ...
      yield* engine.dispatch({
        type: "thread.session.set",
        commandId: CommandId.make("cmd-completed-status-ready"),
        threadId,
        session: {
          threadId,
          status: "ready",
