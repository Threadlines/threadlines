...
import { sanitizeBranchFragment } from "@threadlines/shared/git";
import { detectSourceControlProviderFromRemoteUrl } from "@threadlines/shared/sourceControl";
...
export class BitbucketApi extends Context.Service<BitbucketApi, BitbucketApiShape>()(
  "threadlines/source-control/BitbucketApi",
) {}
...
function checkoutBranchName(input: {
  ...
  return `threadlines/pr-${input.pullRequestId}/${sanitizeBranchFragment(input.headBranch)}`;
}
