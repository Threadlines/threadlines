...
const emitThoughts = process.env.T3_ACP_EMIT_THOUGHTS === "1";
...
const program = Effect.gen(function* () {
  ...
  yield* agent.handlePrompt((request) =>
    Effect.gen(function* () {
      ...
      if (emitThoughts) {
        // fx's shape: a run of thought chunks, then the reply, then the
        // context fill for the turn.
        for (const text of ["Checking the ", "workspace first."]) {
          yield* agent.client.sessionUpdate({
            sessionId: requestedSessionId,
        ...
        yield* agent.client.sessionUpdate({
          sessionId: requestedSessionId,
          update: { sessionUpdate: "usage_update", used: 11_767, size: 256_000 },
        });
        return { stopReason: "end_turn" };
      }
