...
it.live("completes a checkpoint revert without an active provider session", () =>
  withHarness((harness) =>
    Effect.gen(function* () {
      yield* seedProjectAndThread(harness);
      ...
      // Session-less reverts resolve the workspace from thread/project
      // config; only provider conversation rollback needs a live session.
      const thread = yield* harness.waitForThread(THREAD_ID, (entry) =>
      ...
      assert.equal(
        (outcomeActivity?.payload as { readonly mode?: string } | undefined)?.mode,
        "selective",
      );
      assert.equal(
