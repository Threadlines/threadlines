...
import {
  ...
  readFileForStash,
} from "./stashImageCompression";

/**
 * The unit environment has no real canvas or codec, so the re-encode path is
 * exercised with stubbed `createImageBitmap` + `OffscreenCanvas`. The encoder
 * stub returns a payload whose size scales with quality, mirroring how a real
 * encoder shrinks as quality drops: enough to verify the ladder logic and
 * budget enforcement without pulling in a native canvas.
 */

const originalCreateImageBitmap = globalThis.createImageBitmap;
const originalOffscreenCanvas = globalThis.OffscreenCanvas;

function makeFile(sizeBytes: number, type = "image/png", name = "shot.png"): File {
...
describe("readFileForStash", () => {
  it("stores a non-image file verbatim when it fits the budget", async () => {
    const result = await readFileForStash(makeFile(1024, "application/pdf", "spec.pdf"));

    expect(result.ok).toBe(true);
    expect(result.ok && result.image.recompressed).toBe(false);
    expect(result.ok && result.image.mimeType).toBe("application/pdf");
    expect(result.ok && result.image.dataUrl.startsWith("data:application/pdf")).toBe(true);
  });
