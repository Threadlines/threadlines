...
describe("SidebarVersionTag", () => {
  ...
  it("keeps the installed build label independent of the nightly update preference", async () => {
    stubDesktopBridge({
    ...
    await versionChip().hover();
    await expect.element(page.getByText(APP_BUILD_CHANNEL_LABEL)).toBeVisible();
    await expect.element(page.getByText(/Nightly updates/)).not.toBeInTheDocument();
  });
