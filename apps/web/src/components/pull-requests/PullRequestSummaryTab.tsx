...
import { ExternalLinkIcon, MessagesSquareIcon, PencilIcon, WandSparklesIcon } from "lucide-react";
    ...
    <div className="h-full min-h-0 overflow-y-auto px-4 py-4">
      ...
      <section className="mt-6">
        <div className="flex min-w-0 items-center gap-2">
          <h3 className={cn(SECTION_LABEL_CLASS, "mb-0 flex-1")}>
        ...
        {detail.reviewers.length === 0 ? (
          <p className="mt-2 text-xs text-muted-foreground/55">No reviewers.</p>
        ) : (
          <div className="mt-2 flex flex-col divide-y divide-border/50">
            {detail.reviewers.map((reviewer) => (
              <div
                key={`${reviewer.kind}:${reviewer.id}`}
                className="flex min-w-0 items-center gap-1.5 py-1.5 text-xs"
              >
                <span className="min-w-0 truncate text-foreground/85">{reviewer.login}</span>
                {reviewer.kind === "team" ? (
                  <span className="shrink-0 text-muted-foreground/55">team</span>
                ) : null}
                <MetaSeparator />
                <span className="shrink-0 text-muted-foreground/60">
                  {REVIEW_STATE_WORDS[reviewer.state]}
                </span>
              </div>
            ))}
          </div>
...
function PullRequestDescription({
  ...
  return (
    <section className="group/description group/reactions">
      <div className="flex min-w-0 items-center gap-2">
        <h3 className={cn(SECTION_LABEL_CLASS, "mb-0 flex-1")}>Description</h3>
        {canEdit && !editing ? (
          <Button
            variant="ghost"
            size="icon-xs"
