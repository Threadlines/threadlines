...
/** Same chip shape, amber warning tint: the selected provider cannot resolve
 *  this skill, so the token would be sent as literal words. */
export const COMPOSER_INLINE_STALE_SKILL_CHIP_CLASS_NAME =
  "inline-flex max-w-full select-none items-center gap-1 rounded-md border border-amber-500/30 bg-amber-500/12 px-1.5 py-px font-medium text-[12px] leading-[1.1] text-amber-700 align-middle dark:text-amber-300";
...
export const COMPOSER_INLINE_CHIP_DISMISS_BUTTON_CLASS_NAME =
  "ml-0.5 inline-flex size-3.5 shrink-0 cursor-pointer items-center justify-center rounded-sm text-muted-foreground/72 transition-colors hover:bg-foreground/6 hover:text-foreground focus-ring";
