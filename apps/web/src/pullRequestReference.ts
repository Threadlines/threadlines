...
export function parsePullRequestReference(input: string): string | null {
  ...
  const urlMatch =
    parsePullRequestUrl(normalizedInput) !== null ||
    GITLAB_MERGE_REQUEST_URL_PATTERN.test(normalizedInput) ||
    AZURE_DEVOPS_PULL_REQUEST_URL_PATTERN.test(normalizedInput);
