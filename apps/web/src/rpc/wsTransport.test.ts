...
describe("WsTransport", () => {
  ...
  it("tracks initial connection failures for the app error state", async () => {
    ...
    await waitFor(() => {
      expect(getWsConnectionStatus()).toMatchObject({
        ...
        hasConnected: false,
        lastError: "Unable to connect to the Threadlines server WebSocket.",
        phase: "disconnected",
      });
    });
  ...
  it("re-subscribes live stream listeners after the protocol replaces a closed socket", async () => {
    const transport = createTransport("ws://localhost:3020");
    const listener = vi.fn();
    const onResubscribe = vi.fn();
  ...
  it("keeps retrying stream subscriptions after application-level failures", async () => {
    const transport = createTransport("ws://localhost:3020");
