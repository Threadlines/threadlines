...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit THREADLINES_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "THREADLINES_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid THREADLINES_PORT_OFFSET"));
      }),
    );
  });

  describe("createDevRunnerEnv", () => {
    it.effect("defaults Threadlines home to ~/.threadlines when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.THREADLINES_HOME, path.resolve(NodeOS.homedir(), ".threadlines"));
        assert.equal(env.BADCODE_HOME, undefined);
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        const env = yield* createDevRunnerEnv({
          ...
          serverOffset: 0,
          webOffset: 0,
          threadlinesHome: "/tmp/custom-threadlines",
          noBrowser: true,
        ...
        assert.equal(env.THREADLINES_HOME, path.resolve("/tmp/custom-threadlines"));
        assert.equal(env.BADCODE_HOME, undefined);
        assert.equal(env.T3CODE_HOME, undefined);
        assert.equal(env.THREADLINES_PORT, "4222");
        assert.equal(env.BADCODE_PORT, undefined);
        assert.equal(env.T3CODE_PORT, undefined);
        assert.equal(env.VITE_HTTP_URL, "http://localhost:4222");
        assert.equal(env.VITE_WS_URL, "ws://localhost:4222");
        assert.equal(env.THREADLINES_NO_BROWSER, "1");
        assert.equal(env.BADCODE_NO_BROWSER, undefined);
        assert.equal(env.T3CODE_NO_BROWSER, undefined);
        assert.equal(env.THREADLINES_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.BADCODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, undefined);
        assert.equal(env.T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, undefined);
        assert.equal(env.THREADLINES_LOG_WS_EVENTS, "1");
        assert.equal(env.BADCODE_LOG_WS_EVENTS, undefined);
        assert.equal(env.T3CODE_LOG_WS_EVENTS, undefined);
        assert.equal(env.THREADLINES_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.THREADLINES_MODE, "web");
        assert.equal(env.T3CODE_MODE, undefined);
        assert.equal(env.THREADLINES_LOG_WS_EVENTS, undefined);
        assert.equal(env.BADCODE_LOG_WS_EVENTS, undefined);
        assert.equal(env.T3CODE_LOG_WS_EVENTS, undefined);
      }),
    );
    ...
    it.effect("forwards explicit websocket logging false without coercing it away", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.THREADLINES_LOG_WS_EVENTS, "0");
        assert.equal(env.BADCODE_LOG_WS_EVENTS, undefined);
        assert.equal(env.T3CODE_LOG_WS_EVENTS, undefined);
      }),
    );

    it.effect("uses custom threadlinesHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          ...
          serverOffset: 0,
          webOffset: 0,
          threadlinesHome: "/tmp/my-threadlines",
          noBrowser: undefined,
          autoBootstrapProjectFromCwd: undefined,
          logWebSocketEvents: undefined,
          host: undefined,
        ...
        assert.equal(env.THREADLINES_HOME, path.resolve("/tmp/my-threadlines"));
        assert.equal(env.BADCODE_HOME, undefined);
        assert.equal(env.T3CODE_HOME, undefined);
      }),
    );
    ...
    it.effect("pins desktop dev to a stable backend port and websocket url", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.THREADLINES_MODE, undefined);
        assert.equal(env.BADCODE_MODE, undefined);
        assert.equal(env.T3CODE_MODE, undefined);
        assert.equal(env.THREADLINES_NO_BROWSER, undefined);
        assert.equal(env.BADCODE_NO_BROWSER, undefined);
        assert.equal(env.T3CODE_NO_BROWSER, undefined);
        assert.equal(env.THREADLINES_HOST, undefined);
        assert.equal(env.BADCODE_HOST, undefined);
        assert.equal(env.T3CODE_HOST, undefined);
    ...
    it.effect("defaults dev server mode to the higher backend port range", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.THREADLINES_PORT, "13773");
        assert.equal(env.BADCODE_PORT, undefined);
        assert.equal(env.T3CODE_PORT, undefined);
