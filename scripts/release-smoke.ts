...
files:
  - url: BadCode-9.9.9-smoke.0-arm64.zip
    sha512: arm64zip
    size: 125621344
  - url: BadCode-9.9.9-smoke.0-arm64.dmg
    sha512: arm64dmg
    size: 131754935
path: BadCode-9.9.9-smoke.0-arm64.zip
sha512: arm64zip
...
`,
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: BadCode-9.9.9-smoke.0-x64.zip
    sha512: x64zip
    size: 132000112
  - url: BadCode-9.9.9-smoke.0-x64.dmg
    sha512: x64dmg
    size: 138148807
path: BadCode-9.9.9-smoke.0-x64.zip
sha512: x64zip
...
function writeWindowsManifestFixtures(
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: BadCode-9.9.9-smoke.0-arm64.exe
    sha512: arm64exe
    size: 126621344
  - url: BadCode-9.9.9-smoke.0-arm64.exe.blockmap
    sha512: arm64blockmap
    size: 152344
path: BadCode-9.9.9-smoke.0-arm64.exe
sha512: arm64exe
releaseDate: '2026-03-08T10:32:14.587Z'
`,
  );
  ...
  writeFileSync(
    ...
    `version: 9.9.9-smoke.0
files:
  - url: BadCode-9.9.9-smoke.0-x64.exe
    sha512: x64exe
    size: 132000112
  - url: BadCode-9.9.9-smoke.0-x64.exe.blockmap
    sha512: x64blockmap
    size: 160112
path: BadCode-9.9.9-smoke.0-x64.exe
sha512: x64exe
releaseDate: '2026-03-08T10:36:07.540Z'
`,
  );
...
function mergeWindowsManifestFixtures(targetRoot: string): void {
  const assetDirectory = resolve(targetRoot, "release-assets");
  const x64ManifestNames = readdirSync(assetDirectory).filter(
    (entry) => entry.endsWith("-win-x64.yml") && !entry.startsWith("builder-debug-"),
  );

  if (x64ManifestNames.length === 0) {
    throw new Error("No Windows updater manifests found to merge.");
  }

  for (const x64ManifestName of x64ManifestNames) {
    const x64ManifestPath = resolve(assetDirectory, x64ManifestName);
...
try {
  ...
  assertContains(
    nightlyReleaseMetadata,
    "name=BadCode Nightly 9.9.10-nightly.20260413.321 (abcdef123456)",
  ...
  assertContains(
    mergedManifest,
    "BadCode-9.9.9-smoke.0-arm64.zip",
    "Merged manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedManifest,
    "BadCode-9.9.9-smoke.0-x64.zip",
  ...
  assertContains(
    ...
    "BadCode-9.9.9-smoke.0-arm64.exe",
    "Merged Windows manifest is missing the arm64 asset.",
  );
  assertContains(
    mergedWindowsManifest,
    "BadCode-9.9.9-smoke.0-x64.exe",
