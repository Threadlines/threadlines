...
function buildPrompt(input: GenerateReleaseContentInput): string {
  ...
  return [
    ...
    "- Write for a user reading their feed, not a developer reading a diff: say what the person can now do or what they will see on screen, in plain words. 'Warnings collapse into one slim row above the message box' beats 'Docked composer notices'.",
    "- Never use internal feature or component names in user-facing copy. If a name is unavoidable, describe how the thing looks or behaves instead.",
    "- Never use em dashes in any copy. Use a period, comma, colon, or parentheses.",
    ...
    "- The social post should contain at most four bullets, each a plain phrase a reader who has never opened the app would understand, ideally under 60 characters, and must only mention claims present in the highlights or smaller improvements.",
    "",
