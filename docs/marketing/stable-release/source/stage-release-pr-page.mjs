...
async function stage(mainDiff) {
  ...
  const serverState = await import("/src/rpc/serverState.ts");
  const serverConfigBaseline = serverState.getServerConfig();
  if (serverConfigBaseline)
    serverState.setServerConfigSnapshot({
      ...serverConfigBaseline,
      providers: serverConfigBaseline.providers.map((provider) => ({
        ...provider,
        status: "ready",
        statusReason: null,
        message: null,
  ...
  function apply() {
    const latestConfig = serverState.getServerConfig();
    if (latestConfig?.providers.some((provider) => provider.status !== "ready")) {
      serverState.setServerConfigSnapshot({
        ...latestConfig,
        providers: latestConfig.providers.map((provider) => ({
          ...provider,
          status: "ready",
          statusReason: null,
          message: null,
    ...
    for (const [id, shell] of Object.entries(next.threadShellById)) {
      if (id !== main.id && shell.projectId === main.projectId && shell.branch === main.branch) {
        const branch = "capture/previous-review";
        next.threadShellById[id] = { ...shell, branch };
        if (next.sidebarThreadSummaryById[id])
          next.sidebarThreadSummaryById[id] = { ...next.sidebarThreadSummaryById[id], branch };
      }
    }
  ...
  window.threadlinesCaptureStates = {
    kind: "release-pr-page",
    captureRoot: "C:/Users/Public/Documents/Threadlines Release Studio",
    environmentId,
    threadId: main.id,
    ...
    setCheckState(state) {
      if (state !== "pending" && state !== "failure")
        throw new Error("Unsupported capture check state");
      detail = {
        ...detail,
        checksState: state,
            ...
            ? {
                ...
                description:
                  state === "pending"
                    ...
                    : "Headline overlaps the download button at phone width",
              }
            : check.name === "Build"
              ? { ...check, status: "success", description: "Passed" }
              : check,
        ),
      };
      lists.open = {
        ...lists.open,
        entries: lists.open.entries.map((item) =>
          item.number === number ? { ...item, checksState: state } : item,
        ),
      };
      apply();
    },
    stop() {
      if (serverConfigBaseline) serverState.setServerConfigSnapshot(serverConfigBaseline);
      clearInterval(timer);
      clearTimeout(expiry);
