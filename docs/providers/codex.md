...
This guide is for people who want to use more than one Codex account in BadCode.
...
In BadCode Settings, name it something obvious:
...
In BadCode Settings, add another Codex provider:
...
BadCode shows the authenticated email for providers that report one. Emails are blurred by default;
...
If you add a third Codex provider with a completely different `CODEX_HOME path`, BadCode treats it
