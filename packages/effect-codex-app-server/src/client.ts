...
export interface CodexAppServerClientShape {
  ...
  readonly handleServerRequest: <M extends CodexRpc.ServerRequestMethod>(
    ...
    handler: (
      ...
      metadata: { readonly id: CodexProtocol.CodexAppServerIncomingRequest["id"] },
    ...
    if (request.method in CodexRpc.SERVER_REQUEST_PARAMS) {
      ...
      return decodeOptionalPayload(method, payloadSchema, request.params).pipe(
        Effect.flatMap((decoded) =>
          runHandler(
            handler ? (payload) => handler(payload, { id: request.id }) : undefined,
            decoded,
            method,
          ),
        ),
...
// Codex CLI diagnostics can exceed the child stderr pipe capacity; without a
// reader the child blocks mid-write and JSON-RPC responses never arrive.
