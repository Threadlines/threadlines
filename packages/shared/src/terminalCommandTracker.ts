...
function splitCommandHead(command: string): { head: string; rest: string } | null {
  const normalized = command.replace(/\s+/g, " ").trim();
  if (normalized.length === 0) return null;
  ...
  if (quote === `"` || quote === "'") {
    ...
    if (closingIndex > 0) {
      return {
        head: normalized.slice(1, closingIndex),
        rest: normalized.slice(closingIndex + 1).trim(),
      };
    }
  }

  const whitespaceMatch = /\s/.exec(normalized);
  if (!whitespaceMatch) {
    return { head: normalized, rest: "" };
  }

  return {
    head: normalized.slice(0, whitespaceMatch.index),
    rest: normalized.slice(whitespaceMatch.index + 1).trim(),
  };
}

function stripExecutablePath(commandHead: string): string {
  const normalized = commandHead.replaceAll("\\", "/");
  const basename = normalized.split("/").findLast((part) => part.length > 0) ?? commandHead;
  return basename.replace(/\.(?:exe|cmd|bat|ps1)$/i, "") || basename;
}

export function normalizeTerminalActivityCommand(command: string): string | null {
  const parsed = splitCommandHead(command);
  if (!parsed) return null;

  const head = stripExecutablePath(parsed.head);
  return normalizeSubmittedTerminalCommand(
    parsed.rest.length > 0 ? `${head} ${parsed.rest}` : head,
  );
}
