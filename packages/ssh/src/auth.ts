...
export class SshPasswordPrompt extends Context.Service<SshPasswordPrompt, SshPasswordPromptShape>()(
  "@threadlines/ssh/auth/SshPasswordPrompt",
) {
...
const SSH_ASKPASS_DIR_NAME = "threadlines-ssh-askpass";
...
# Invoked by ssh via SSH_ASKPASS when Threadlines re-runs ssh with a cached password
# from the renderer's in-app prompt. We never expose a native dialog here - if
# THREADLINES_SSH_AUTH_SECRET is missing, that's a caller bug and we fail loudly.
if [ "\${THREADLINES_SSH_AUTH_SECRET+x}" = "x" ]; then
  printf "%s\\n" "$THREADLINES_SSH_AUTH_SECRET"
  exit 0
fi
printf 'Threadlines ssh-askpass invoked without THREADLINES_SSH_AUTH_SECRET.\\n' >&2
exit 1
`;
...
export const ASKPASS_WINDOWS_SCRIPT = `# Invoked by ssh via SSH_ASKPASS (through ssh-askpass.cmd) when Threadlines re-runs\r
# ssh with a cached password from the renderer's in-app prompt. We never expose\r
# a native dialog here - if THREADLINES_SSH_AUTH_SECRET is missing, that's a caller bug\r
# and we fail loudly.\r
if ($null -ne $env:THREADLINES_SSH_AUTH_SECRET) {\r
  [Console]::Out.WriteLine($env:THREADLINES_SSH_AUTH_SECRET)\r
  exit 0\r
}\r
[Console]::Error.WriteLine("Threadlines ssh-askpass invoked without THREADLINES_SSH_AUTH_SECRET.")\r
exit 1\r
`;
...
export const getDefaultSshAskpassDirectory = Effect.fn("ssh/auth.getDefaultSshAskpassDirectory")(
  function* () {
    ...
    const parentDirectory = yield* fs.makeTempDirectory({ prefix: "threadlines-ssh-runtime-" });
...
> {
  ...
  return {
    ...
    ...(input.authSecret === undefined
      ? {}
      : { THREADLINES_SSH_AUTH_SECRET: input.authSecret ?? "" }),
    ...(platform === "win32" || baseEnv.DISPLAY ? {} : { DISPLAY: "threadlines" }),
  };
});
...
export function isSshAuthFailure(error: unknown): boolean {
  ...
  const permissionMarker = "permission denied (";
  const permissionStart = normalized.indexOf(permissionMarker);
  const permissionEnd =
    permissionStart === -1
      ? -1
      : normalized.indexOf(")", permissionStart + permissionMarker.length);
  const firstMethod =
  ...
  return (
    (firstMethod !== null && recognizedMethods.has(firstMethod)) ||
