...
import { extractJsonObject, fromLenientJson } from "@threadlines/shared/schemaJson";
...
export interface RemoteThreadlinesRunnerOptions {
...
function sshRunnerLogFields(runner: RemoteThreadlinesRunnerOptions | undefined) {
...
remote_node_satisfies_engine() {
  THREADLINES_NODE_ENGINE_RANGE=@@THREADLINES_NODE_ENGINE_RANGE@@
  if [ -z "$THREADLINES_NODE_ENGINE_RANGE" ]; then
    return 0
  fi
  node - "$THREADLINES_NODE_ENGINE_RANGE" <<'NODE'
@@THREADLINES_NODE_ENGINE_CHECK_SCRIPT@@
NODE
}
...
ensure_remote_node_path() {
  ...
  if ! command -v node >/dev/null 2>&1 && [ -d "$NVM_DIR/versions/node" ]; then
    for THREADLINES_NODE_BIN in "$NVM_DIR"/versions/node/*/bin; do
      if [ -x "$THREADLINES_NODE_BIN/node" ]; then
        PATH="$THREADLINES_NODE_BIN:$PATH"
        export PATH
      fi
    done
  fi
...
set -eu
@@THREADLINES_NODE_ENV_SCRIPT@@
ensure_remote_node_path || true
THREADLINES_NODE_SCRIPT_PATH=@@THREADLINES_NODE_SCRIPT_PATH@@
if [ -n "$THREADLINES_NODE_SCRIPT_PATH" ]; then
  ...
  exec node "$THREADLINES_NODE_SCRIPT_PATH" "$@"
fi
if command -v threadlines >/dev/null 2>&1; then
  exec threadlines "$@"
fi
if command -v npx >/dev/null 2>&1; then
  exec npx --yes @@THREADLINES_PACKAGE_SPEC@@ "$@"
fi
if command -v npm >/dev/null 2>&1; then
  exec npm exec --yes @@THREADLINES_PACKAGE_SPEC@@ -- "$@"
fi
printf 'Remote host is missing the Threadlines CLI and could not install @@THREADLINES_PACKAGE_SPEC@@ because node/npm/npx are unavailable on PATH. Install Node or configure a supported version manager for non-interactive shells.\\n' >&2
exit 1
`;
...
@@THREADLINES_NODE_ENV_SCRIPT@@
STATE_KEY="$1"
STATE_DIR="$HOME/.threadlines/ssh-launch/$STATE_KEY"
DEFAULT_SERVER_HOME="$HOME/.threadlines"
DEFAULT_RUNTIME_FILE="$DEFAULT_SERVER_HOME/userdata/server-runtime.json"
PORT_FILE="$STATE_DIR/port"
PID_FILE="$STATE_DIR/pid"
MANAGED_FILE="$STATE_DIR/managed"
LOG_FILE="$STATE_DIR/server.log"
RUNNER_FILE="$STATE_DIR/run-threadlines.sh"
RUNNER_NEXT="$STATE_DIR/run-threadlines.next.$$"
mkdir -p "$STATE_DIR"
cleanup_runner_next() {
  rm -f "$RUNNER_NEXT"
}
...
pick_port() {
  node - "$PORT_FILE" "@@THREADLINES_DEFAULT_REMOTE_PORT@@" "@@THREADLINES_REMOTE_PORT_SCAN_WINDOW@@" <<'NODE'
@@THREADLINES_PICK_PORT_SCRIPT@@
NODE
}
wait_ready() {
  node - "$REMOTE_PORT" "$1" "@@THREADLINES_READY_PROBE_TIMEOUT_MS@@" <<'NODE'
@@THREADLINES_WAIT_READY_SCRIPT@@
NODE
}
wait_for_pid_exit() {
  PID_TO_WAIT="$1"
...
if [ -n "$DEFAULT_REMOTE_PORT" ]; then
  ...
  if wait_ready "@@THREADLINES_REUSE_READY_TIMEOUT_MS@@"; then
...
if [ "$REMOTE_MANAGED" = "external" ]; then
  if [ -z "$REMOTE_PORT" ] || ! wait_ready "@@THREADLINES_REUSE_READY_TIMEOUT_MS@@"; then
    REMOTE_PID=""
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  fi
  ...
  if [ "$RUNNER_CHANGED" -eq 1 ]; then
    ...
    REMOTE_PORT=""
    REMOTE_MANAGED=""
  elif ! wait_ready "@@THREADLINES_REUSE_READY_TIMEOUT_MS@@"; then
    kill "$REMOTE_PID" 2>/dev/null || true
    wait_for_pid_exit "$REMOTE_PID"
    REMOTE_PID=""
    REMOTE_PORT=""
...
if [ -z "$REMOTE_PORT" ]; then
  ...
  nohup env THREADLINES_NO_BROWSER=1 "$RUNNER_FILE" serve --host 127.0.0.1 --port "$REMOTE_PORT" --base-dir "$DEFAULT_SERVER_HOME" >>"$LOG_FILE" 2>&1 < /dev/null &
  REMOTE_PID="$!"
  printf '%s\\n' "$REMOTE_PID" >"$PID_FILE"
  printf '%s\\n' "$REMOTE_PORT" >"$PORT_FILE"
  printf 'managed\\n' >"$MANAGED_FILE"
  if ! wait_ready "@@THREADLINES_READY_TIMEOUT_MS@@"; then
    printf 'Remote Threadlines server did not become ready on 127.0.0.1:%s.\\n' "$REMOTE_PORT" >&2
...
STATE_DIR="$HOME/.threadlines/ssh-launch/@@THREADLINES_STATE_KEY@@"
DEFAULT_SERVER_HOME="$HOME/.threadlines"
RUNNER_FILE="$STATE_DIR/run-threadlines.sh"
mkdir -p "$STATE_DIR"
cat >"$RUNNER_FILE" <<'SH'
...
export function buildRemoteThreadlinesRunnerScript(input?: RemoteThreadlinesRunnerOptions): string {
  const packageSpec = shellSingleQuote(input?.packageSpec?.trim() || "@threadlines/server@latest");
  const nodeScriptPath = input?.nodeScriptPath?.trim() || "";
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_RUNNER_SCRIPT, {
      THREADLINES_PACKAGE_SPEC: packageSpec,
      THREADLINES_NODE_SCRIPT_PATH: shellSingleQuote(nodeScriptPath),
      THREADLINES_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    }),
  );
}

function buildRemoteNodeEnvScript(input?: RemoteThreadlinesRunnerOptions): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(REMOTE_NODE_ENV_SCRIPT, {
      THREADLINES_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      THREADLINES_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(
...
export function buildRemoteLaunchScript(input?: RemoteThreadlinesRunnerOptions): string {
  return applyScriptPlaceholders(REMOTE_LAUNCH_SCRIPT, {
    THREADLINES_NODE_ENV_SCRIPT: buildRemoteNodeEnvScript(input),
    THREADLINES_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteThreadlinesRunnerScript(input)),
    THREADLINES_PICK_PORT_SCRIPT: stripTrailingNewlines(REMOTE_PICK_PORT_SCRIPT),
    THREADLINES_WAIT_READY_SCRIPT: stripTrailingNewlines(REMOTE_WAIT_READY_SCRIPT),
    THREADLINES_DEFAULT_REMOTE_PORT: String(DEFAULT_REMOTE_PORT),
    THREADLINES_REMOTE_PORT_SCAN_WINDOW: String(REMOTE_PORT_SCAN_WINDOW),
    THREADLINES_READY_TIMEOUT_MS: String(REMOTE_READY_TIMEOUT_MS),
    THREADLINES_REUSE_READY_TIMEOUT_MS: String(REMOTE_REUSE_READY_TIMEOUT_MS),
    THREADLINES_READY_PROBE_TIMEOUT_MS: String(SSH_READY_PROBE_TIMEOUT_MS),
  });
}

export function buildRemotePairingScript(
  target: DesktopSshEnvironmentTarget,
  input?: RemoteThreadlinesRunnerOptions,
): string {
  return applyScriptPlaceholders(REMOTE_PAIRING_SCRIPT, {
    THREADLINES_STATE_KEY: remoteStateKey(target),
    THREADLINES_RUNNER_SCRIPT: stripTrailingNewlines(buildRemoteThreadlinesRunnerScript(input)),
  });
}
...
export const launchOrReuseRemoteServer = Effect.fn("ssh/tunnel.launchOrReuseRemoteServer")(
  function* (
    ...
    runner?: RemoteThreadlinesRunnerOptions,
  ): Effect.fn.Return<
...
const makeSshEnvironmentManager = Effect.fn("ssh/tunnel.SshEnvironmentManager.make")(function* (
  ...
  const createTunnelEntry = Effect.fn("ssh/tunnel.ensureTunnelEntry.create")(function* (input: {
    readonly key: string;
    readonly resolvedTarget: DesktopSshEnvironmentTarget;
    readonly runner?: RemoteThreadlinesRunnerOptions;
...
>()("@threadlines/ssh/tunnel/SshEnvironmentManager") {
