...
/** Non-image attachment kinds. `pdf` maps to native provider document
 *  support where available; `text` covers plain text, markdown, and CSV
 *  payloads that providers receive as text documents. The server derives
 *  the kind from the validated MIME type — clients never choose it. */
export const ChatFileAttachmentKind = Schema.Literals(["pdf", "text"]);
export type ChatFileAttachmentKind = typeof ChatFileAttachmentKind.Type;
...
export class CodexInlineVisualizationReadError extends Schema.TaggedError<CodexInlineVisualizationReadError>()(
...
/** Mirrors the Codex app-server ThreadGoal status values verbatim so no
 *  lossy mapping sits between the provider and the read model. */
...
/**
 * A long-horizon goal attached to a thread (Codex goal mode). Authoritative
 * state comes from provider `goal.updated` notifications; commands only
 * express intent.
 */
...
export const OrchestrationThread = Schema.Struct({
   ...
   * the configured checkout (worktreePath ?? project workspace root) — e.g.
   * the agent entered a git worktree mid-session. Null while the session
   * works where the thread was configured to.
   */
...
export const OrchestrationThreadShell = Schema.Struct({
  ...
  branch: Schema.NullOr(TrimmedNonEmptyString),
  worktreePath: Schema.NullOr(TrimmedNonEmptyString),
  /** See OrchestrationThread.effectiveCwd. */
  effectiveCwd: Schema.NullOr(TrimmedNonEmptyString).pipe(
    Schema.withDecodingDefault(Effect.succeed(null)),
  ),
...
/** Intent: attach or update a goal on the thread's provider session. Partial —
 *  omitted fields keep their current provider-side values (pause/resume send
 *  only `status`). Authoritative state flows back via `goal.updated`. */
...
/** Intent: detach the thread's goal. */
...
/** Provider-internal: record provider-reported goal state (null when the
 *  provider cleared it). Emitted by runtime ingestion off `goal.updated` /
 *  `goal.cleared` events. */
const ThreadGoalStateSetCommand = Schema.Struct({
  type: Schema.Literal("thread.goal.state.set"),
  commandId: CommandId,
  threadId: ThreadId,
...
/**
 * Provider-internal: record the session's observed working directory when it
 * diverges from the thread's configured checkout (null when it returns to
 * it). Emitted by runtime ingestion off `session.cwd.changed` events.
 */
const ThreadEffectiveCwdSetCommand = Schema.Struct({
  type: Schema.Literal("thread.effective-cwd.set"),
  commandId: CommandId,
  threadId: ThreadId,
...
export const ThreadGoalStateSetPayload = Schema.Struct({
  threadId: ThreadId,
  goal: Schema.NullOr(OrchestrationThreadGoal),
  updatedAt: IsoDateTime,
});
...
export const OrchestrationEvent = Schema.Union([
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.effective-cwd-set"),
    payload: ThreadEffectiveCwdSetPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.goal-state-set"),
    payload: ThreadGoalStateSetPayload,
  }),
  Schema.Struct({
    ...EventBaseFields,
...
export class OrchestrationGetRevertPlanError extends Schema.TaggedError<OrchestrationGetRevertPlanError>()(
